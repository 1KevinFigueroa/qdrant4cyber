...
You are using the `pinecone-index` Docker image, which exposes a single index with no control-plane API. The script is already designed for this — make sure you are running the latest version of `nmap_to_pinecone-MiniLM-L6.py`.
...
```
.
├── nmap_to_pinecone-MiniLM-L6.py            # Main ingest script
